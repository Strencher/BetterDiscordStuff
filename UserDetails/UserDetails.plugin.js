...
function buildPlugin([BasePlugin, PluginApi]) {
	...
	(() => {
		...
		(() => {
			...
			function dynamicAgo(date1, date2) {
				...
				if (months > 12) {
					...
					} else if (days > 0) return `${Math.floor(days)} day${days > 1 ? "s" : ""}`;
					else {
						const seconds = date2.getSeconds() - date1.getSeconds();
						const minutes = Math.floor(seconds / 60);
						const hours = Math.floor(minutes / 60);
						if (hours > 1) return `${hours} hours`;
						else if (1 == hours) return "an hour";
						else if (minutes > 1) return `${minutes} minutes`;
						else if (1 == minutes) return "a minute";
			...
			function parseTime(format, date) {
				...
				return format.replace(/\$timelabel/g, date.getHours() >= 12 ? "PM" : "AM").replace(/\$ago/g, dynamicAgo(today, date)).replace(/\$daysago/g, daysago.toString()).replace(/\$dayname/g, date.toLocaleDateString("default", {
					weekday: "short",
					hour12
