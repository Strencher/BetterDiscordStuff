...
const BadgesModule = WebpackModules.getByProps("TextBadge");
...
const IconSetting = () => {
    const colored = useStateFromStores([Settings], () => Settings.get("coloredConnectionsIcons", true));
    const forceUpdate = Utilities.useForceUpdate(); 
    const shownIcons = useStateFromStores([Settings], () => Settings.get("shownConnections", Object.fromEntries(Connections.map(e => [e.type, true]))));
                    ...
                    activeItem === "main"
                        ? Pages.map((page, index) => (
                            <div className={styles.cardItem}>
                                ...
                                />
                                {daysAgo(new Date(), new Date(page.added)) < 60 && page.added != null && (
                                    <BadgesModule.TextBadge text={Messages.NEW} color={Colors.STATUS_RED_500} className={styles.textBadge} />
                                )}
                            </div>
                        ))
