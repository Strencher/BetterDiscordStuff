/**
 * @name PronounDB
 * @author Strencher, Kyza, MinekPo1
...
const config = {
	...
	"info": {
		...
		"authors": [{
			...
			{
				"name": "MinekPo1",
				"discord_id": "349810624273842176",
				"github_username": "MinekPo1"
			}
		],
		"description": "Shows you the pronoun of a user right next to their name. Pronouns by https://pronoundb.org. Source code can be found in the ./src folder of the github repo.",
...
function buildPlugin([BasePlugin, PluginApi]) {
	...
	(() => {
		...
		var __webpack_modules__ = {
			...
			577: (module, __webpack_exports__, __webpack_require__) => {
				__webpack_require__.d(__webpack_exports__, {
					Z: () => __WEBPACK_DEFAULT_EXPORT__
				});
				var _bdbuilder_node_modules_css_loader_dist_runtime_api_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(118);
			...
			686: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
				...
				class Settings {
					static __initStatic() {
						this.updater = new(external_events_default());
					}
				...
				var _global = "undefined" !== typeof self ? self : "undefined" !== typeof window ? window : global;
				if ("undefined" !== typeof Promise && !_global.Promise) _global.Promise = Promise;
				function extend(obj, extension) {
					if ("object" !== typeof extension) return obj;
				...
				function props(proto, extension) {
					if ("function" === typeof extension) extension = extension(getProto(proto));
					keys(extension).forEach((function(key) {
				...
				function setByKeyPath(obj, keyPath, value) {
					if (!obj || void 0 === keyPath) return;
					if ("isFrozen" in Object && Object.isFrozen(obj)) return;
					if ("string" !== typeof keyPath && "length" in keyPath) {
				...
				function delByKeyPath(obj, keyPath) {
					if ("string" === typeof keyPath) setByKeyPath(obj, keyPath, void 0);
				...
				function deepClone(any) {
					...
					if (isArray(any)) {
						rv = [];
						for (var i = 0, l = any.length; i < l; ++i) rv.push(deepClone(any[i]));
					} else if (intrinsicTypes.indexOf(any.constructor) >= 0) rv = any;
					else {
						rv = any.constructor ? Object.create(any.constructor.prototype) : {};
						for (var prop in any)
							if (hasOwn(any, prop)) rv[prop] = deepClone(any[prop]);
					}
					return rv;
				}
				...
				var getValueOf = function(val, type) {
					return "Array" === type ? "" + val.map((function(v) {
						return getValueOf(v, toStringTag(v));
					})) : "ArrayBuffer" === type ? "" + new Uint8Array(val) : "Date" === type ? val.getTime() : ArrayBuffer.isView(val) ? "" + new Uint8Array(val.buffer) : val;
				};
							...
							if ("object" === typeof ap && "object" === typeof bp && ap && bp) {
								...
								if (apTypeName === bpTypeName)
									if (intrinsicTypeNameSet[apTypeName]) {
										if (getValueOf(ap, apTypeName) !== getValueOf(bp, bpTypeName)) rv[prfx + prop] = b[prop];
									} else getObjectDiff(ap, bp, rv, prfx + prop + ".");
								else rv[prfx + prop] = b[prop];
				...
				var iteratorSymbol = "undefined" !== typeof Symbol && Symbol.iterator;
				...
				function getArrayOf(arrayLike) {
					...
					if (1 === arguments.length) {
						...
						if (this === NO_CHAR_ARRAY && "string" === typeof arrayLike) return [arrayLike];
						if (it = getIteratorOf(arrayLike)) {
							a = [];
							while (x = it.next(), !x.done) a.push(x.value);
							return a;
						}
						if (null == arrayLike) return [arrayLike];
						i = arrayLike.length;
						if ("number" === typeof i) {
							a = new Array(i);
				...
				var isAsyncFunction = "undefined" !== typeof Symbol ? function(fn) {
					return "AsyncFunction" === fn[Symbol.toStringTag];
				} : function() {
					return false;
				};
				var debug = "undefined" !== typeof location && /^(http|https):\/\/(localhost|127\.0\.0\.1)/.test(location.href);
				...
				function prettyStack(exception, numIgnoredFrames) {
					...
					if (0 === stack.indexOf(exception.name)) numIgnoredFrames += (exception.name + exception.message).split("\n").length;
				...
				var exceptions = errorList.reduce((function(obj, name) {
					...
					function DexieError(msgOrInner, inner) {
						...
						if (!msgOrInner) {
							...
							this.inner = null;
						} else if ("string" === typeof msgOrInner) {
							this.message = msgOrInner + (!inner ? "" : "\n " + inner);
							this.inner = inner || null;
						} else if ("object" === typeof msgOrInner) {
				...
				var fullNameExceptions = errorList.reduce((function(obj, name) {
					if (-1 === ["Syntax", "Type", "Range"].indexOf(name)) obj[name + "Error"] = exceptions[name];
					return obj;
				}), {});
				...
				function pureFunctionChain(f1, f2) {
					if (null == f1 || f1 === mirror) return f2;
					return function(val) {
						return f2(f1(val));
					};
				}
				...
				function hookUpdatingChain(f1, f2) {
					...
					return function(modifications) {
						...
						if (onsuccess) this.onsuccess = this.onsuccess ? callBoth(onsuccess, this.onsuccess) : onsuccess;
						if (onerror) this.onerror = this.onerror ? callBoth(onerror, this.onerror) : onerror;
						return void 0 === res ? void 0 === res2 ? void 0 : res2 : extend(res, res2);
					};
				}
				function reverseStoppableEventChain(f1, f2) {
					if (f1 === nop) return f2;
					return function() {
						if (false === f2.apply(this, arguments)) return false;
				...
				var _a = "undefined" === typeof Promise ? [] : function() {
					var globalP = Promise.resolve();
					if ("undefined" === typeof crypto || !crypto.subtle) return [globalP, globalP.__proto__, globalP];
				...
				function DexiePromise(fn) {
					...
					if ("function" !== typeof fn) {
						...
						if (false === this._state) handleRejection(this, this._value);
						return;
					}
					this._state = null;
					this._value = null;
				...
				props(DexiePromise.prototype, {
					...
					_then: function(onFulfilled, onRejected) {
						...
						var type = arguments[0],
							handler = arguments[1];
						return "function" === typeof type ? this.then(null, (function(err) {
					...
					stack: {
						get: function() {
							...
							try {
								...
								if (null !== this._state) this._stack = stack;
								return stack;
							} finally {
				...
				if ("undefined" !== typeof Symbol && Symbol.toStringTag) setProp(DexiePromise.prototype, Symbol.toStringTag, "Dexie.Promise");
							...
							return newScope((function(resolve, reject) {
								...
								psd.finalize = callBoth((function() {
									...
									run_at_end_of_this_or_next_physical_tick((function() {
										0 === _this.unhandleds.length ? resolve() : reject(_this.unhandleds[0]);
									}));
								}), psd.finalize);
								fn();
				...
				if (NativePromise) {
					if (NativePromise.allSettled) setProp(DexiePromise, "allSettled", (function() {
						...
						return new DexiePromise((function(resolve) {
							if (0 === possiblePromises.length) resolve([]);
					...
					if (NativePromise.any && "undefined" !== typeof AggregateError) setProp(DexiePromise, "any", (function() {
						var possiblePromises = getArrayOf.apply(null, arguments).map(onPossibleParallellAsync);
						return new DexiePromise((function(resolve, reject) {
							if (0 === possiblePromises.length) reject(new AggregateError([]));
							var remaining = possiblePromises.length;
				...
				function executePromiseTask(promise, fn) {
					try {
						fn((function(value) {
							if (null !== promise._state) return;
							if (value === promise) throw new TypeError("A promise cannot be resolved with itself.");
							var shouldExecuteTick = promise._lib && beginMicroTickScope();
							if (value && "function" === typeof value.then) executePromiseTask(promise, (function(resolve, reject) {
				...
				function handleRejection(promise, reason) {
					...
					debug && null !== reason && "object" === typeof reason && !reason._promise && tryCatch((function() {
				...
				function propagateAllListeners(promise) {
					...
					if (0 === numScheduledCalls) {
						++numScheduledCalls;
						asap$1((function() {
							if (0 === --numScheduledCalls) finalizePhysicalTick();
						}), []);
					}
				}
				...
				function callListener(cb, promise, listener) {
					try {
						...
						if (promise._state) ret = cb(value);
							...
							ret = cb(value);
							if (-1 === rejectingErrors.indexOf(value)) markErrorAsHandled(promise);
						}
				...
				function getStack(promise, stacks, limit) {
					...
					if (debug) {
						...
						if (stack && -1 === stacks.indexOf(stack)) stacks.push(stack);
				...
				function decrementExpectedAwaits() {
					...
					if (0 === --task.awaits) task.id = 0;
					task.echoes = task.awaits * ZONE_ECHO_LIMIT;
					return true;
				}
				if (-1 === ("" + nativePromiseThen).indexOf("[native code]")) incrementExpectedAwaits = decrementExpectedAwaits = nop;
				...
				function zoneEnterEcho(targetZone) {
					++totalEchoes;
					if (!task.echoes || 0 === --task.echoes) task.echoes = task.id = 0;
					zoneStack.push(PSD);
				...
				var connections = [];
				var isIEOrEdge = "undefined" !== typeof navigator && /(MSIE|Trident|Edge)/.test(navigator.userAgent);
				...
				function workaroundForUndefinedPrimKey(keyPath) {
					return function(obj) {
						if (void 0 === getByKeyPath(obj, keyPath)) {
							obj = deepClone(obj);
				...
				var Table = function() {
					...
					Table.prototype.update = function(keyOrObject, modifications) {
						if ("object" !== typeof modifications || isArray(modifications)) throw new exceptions.InvalidArgument("Modifications must be an object.");
						if ("object" === typeof keyOrObject && !isArray(keyOrObject)) {
					...
					Table.prototype.bulkAdd = function(objects, keysOrOptions, options) {
						...
						return this._trans("readwrite", (function(trans) {
							...
							return _this.core.mutate({
								...
								if (0 === numFailures) return result;
				...
				function Events(ctx) {
					...
					var rv = function(eventName, subscriber) {
						if (subscriber) {
							...
							return ctx;
						} else if ("string" === typeof eventName) return evs[eventName];
					};
					rv.addEventType = add;
					...
					function add(eventName, chainFunction, defaultFunction) {
						...
						var context = {
							...
							subscribe: function(cb) {
								if (-1 === context.subscribers.indexOf(cb)) {
					...
					function addConfiguredEvents(cfg) {
						keys(cfg).forEach((function(eventName) {
							...
							else if ("asap" === args) var context = add(eventName, mirror, (function fire() {
								var i = arguments.length,
									args = new Array(i);
								while (i--) args[i] = arguments[i];
					...
					if (!ctx.or) return iterate(openCursor(ctx, coreTable, coreTrans), combine(ctx.algorithm, filter), fn, !ctx.keysOnly && ctx.valueMapper);
						...
						var union = function(item, cursor, advance) {
							if (!filter || filter(cursor, advance, (function(result) {
								...
								var key = "" + primaryKey;
								if ("[object ArrayBuffer]" === key) key = "" + new Uint8Array(primaryKey);
								if (!hasOwn(set_1, key)) {
									set_1[key] = true;
				...
				var Collection = function() {
					...
					Collection.prototype.toArray = function(cb) {
						...
						return this._read((function(trans) {
							var ctx = _this._ctx;
							if ("next" === ctx.dir && isPlainKeyRange(ctx, true) && ctx.limit > 0) {
					...
					Collection.prototype.primaryKeys = function(cb) {
						var ctx = this._ctx;
						if ("next" === ctx.dir && isPlainKeyRange(ctx, true) && ctx.limit > 0) return this._read((function(trans) {
							var index = getIndexOrStore(ctx, ctx.table.core.schema);
							return ctx.table.core.query({
								trans,
								values: false,
								...
								var nextChunk = function(offset) {
									...
									return coreTable.getMany({
										...
										for (var i = 0; i < count; ++i) {
											...
											if (false !== modifyer.call(ctx_1, ctx_1.value, ctx_1))
												if (null == ctx_1.value) deleteKeys.push(keys$1[offset + i]);
												else if (!outbound && 0 !== cmp(extractKey(origValue), extractKey(ctx_1.value))) {
					...
					Collection.prototype.delete = function() {
						var ctx = this._ctx,
							range = ctx.range;
						if (isPlainKeyRange(ctx) && (ctx.isPrimKey && !hangsOnDeleteLargeKeyRange || 3 === range.type)) return this._write((function(trans) {
				...
				function nextCasing(key, lowerKey, upperNeedle, lowerNeedle, cmp, dir) {
					...
					if (length < lowerNeedle.length && "next" === dir) return key + upperNeedle.substr(key.length);
					if (length < key.length && "prev" === dir) return key.substr(0, upperNeedle.length);
				...
				function addIgnoreCaseAlgorithm(whereClause, match, needles, suffix) {
					...
					function initDirection(dir) {
						...
						compare = "next" === dir ? simpleCompare : simpleCompareReverse;
					...
					c._addAlgorithm((function(cursor, advance, resolve) {
						...
						if (match(lowerKey, lowerNeedles, firstPossibleNeedle)) return true;
							...
							for (var i = firstPossibleNeedle; i < needlesLen; ++i) {
								...
								if (null === casing && null === lowestPossibleCasing) firstPossibleNeedle = i + 1;
								else if (null === lowestPossibleCasing || compare(lowestPossibleCasing, casing) > 0) lowestPossibleCasing = casing;
							}
							if (null !== lowestPossibleCasing) advance((function() {
				...
				var WhereClause = function() {
					...
					WhereClause.prototype.between = function(lower, upper, includeLower, includeUpper) {
						includeLower = false !== includeLower;
						includeUpper = true === includeUpper;
						try {
							if (this._cmp(lower, upper) > 0 || 0 === this._cmp(lower, upper) && (includeLower || includeUpper) && !(includeLower && includeUpper)) return emptyCollection(this);
					...
					WhereClause.prototype.equals = function(value) {
						if (null == value) return fail(this, INVALID_KEY_ARGUMENT);
						return new this.Collection(this, (function() {
					...
					WhereClause.prototype.startsWith = function(str) {
						if ("string" !== typeof str) return fail(this, STRING_EXPECTED);
					...
					WhereClause.prototype.anyOfIgnoreCase = function() {
						...
						if (0 === set.length) return emptyCollection(this);
						return addIgnoreCaseAlgorithm(this, (function(x, a) {
					...
					WhereClause.prototype.anyOf = function() {
						...
						c._ondirectionchange = function(direction) {
							compare = "next" === direction ? _this._ascending : _this._descending;
							set.sort(compare);
						};
						var i = 0;
						c._addAlgorithm((function(cursor, advance, resolve) {
							...
							if (0 === compare(key, set[i])) return true;
							else {
								advance((function() {
					...
					WhereClause.prototype.noneOf = function() {
						var set = getArrayOf.apply(NO_CHAR_ARRAY, arguments);
						if (0 === set.length) return new this.Collection(this);
						try {
					...
					WhereClause.prototype.inAnyRange = function(ranges, options) {
						...
						var includeLowers = !options || false !== options.includeLowers;
						var includeUppers = options && true === options.includeUppers;
						...
						c._addAlgorithm((function(cursor, advance, resolve) {
							...
							else if (0 === _this._cmp(key, set[rangePos][1]) || 0 === _this._cmp(key, set[rangePos][0])) return false;
							else {
								advance((function() {
				...
				var Transaction = function() {
					...
					Transaction.prototype._lock = function() {
						assert(!PSD.global);
						++this._reculock;
						if (1 === this._reculock && !PSD.global) PSD.lockOwnerFor = this;
						return this;
					};
					...
					Transaction.prototype.waitFor = function(promiseLike) {
						...
						if (root._waitingFor) root._waitingFor = root._waitingFor.then((function() {
							...
							(function spin() {
								...
								if (root._waitingFor) store.get(-1 / 0).onsuccess = spin;
							})();
						}
				...
				function getKeyExtractor(keyPath) {
					if (null == keyPath) return function() {
						return;
					};
					else if ("string" === typeof keyPath) return getSinglePathKeyExtractor(keyPath);
				...
				function getSinglePathKeyExtractor(keyPath) {
					...
					if (1 === split.length) return function(obj) {
						return obj[keyPath];
					};
					else return function(obj) {
						return getByKeyPath(obj, keyPath);
				...
				function getEffectiveKeys(primaryKey, req) {
					if ("delete" === req.type) return req.keys;
					return req.keys || req.values.map(primaryKey.extractKey);
				}
				function getExistingValues(table, req, effectiveKeys) {
					return "add" === req.type ? Promise.resolve(new Array(req.values.length)) : table.getMany({
						trans: req.trans,
						keys: effectiveKeys
					});
				}
				...
				function createDBCore(db, indexedDB, IdbKeyRange, tmpTrans) {
						...
						return {
							schema: {
								...
								tables: tables.map((function(table) {
									...
									if (null != keyPath) indexByKeyPath[getKeyPathAlias(keyPath)] = result.primaryKey;
									return result;
								}))
							},
							hasGetAll: tables.length > 0 && "getAll" in trans.objectStore(tables[0]) && !("undefined" !== typeof navigator && /Safari/.test(navigator.userAgent) && !/(Chrome\/|Edge\/)/.test(navigator.userAgent) && [].concat(navigator.userAgent.match(/Safari\/(\d*)/))[1] < 604)
						};
					}
					function makeIDBKeyRange(range) {
						...
						var idbRange = void 0 === lower ? void 0 === upper ? null : IdbKeyRange.upperBound(upper, !!upperOpen) : void 0 === upper ? IdbKeyRange.lowerBound(lower, !!lowerOpen) : IdbKeyRange.bound(lower, upper, !!lowerOpen, !!upperOpen);
						return idbRange;
					}
					function createDbCoreTable(tableSchema) {
						...
						function mutate(_a) {
							...
							return new Promise((function(resolve, reject) {
								...
								var outbound = null == store.keyPath;
								var isAddOrPut = "put" === type || "add" === type;
								if (!isAddOrPut && "delete" !== type && "deleteRange" !== type) throw new Error("Invalid operation type: " + type);
								var length = (keys$1 || values || {
									length: 1
								}).length;
								if (keys$1 && values && keys$1.length !== values.length) throw new Error("Given keys array must have same length as given values array.");
								...
								var errorHandler = function(event) {
									++numFailures;
									preventDefault(event);
									if (results) results[event.target._reqno] = void 0;
								...
								if ("deleteRange" === type) {
									...
									if (3 === range.type) req = store.clear();
									...
									if (isAddOrPut)
										for (var i = 0; i < length; ++i) {
											req = args2 && void 0 !== args2[i] ? store[type](args1[i], args2[i]) : store[type](args1[i]);
											req._reqno = i;
											if (results && void 0 === results[i]) req.onsuccess = setResult;
							...
							return function(request) {
								return new Promise((function(resolve, reject) {
									...
									var trans = request.trans,
										...
										limit = request.limit,
										query = request.query;
									var nonInfinitLimit = limit === 1 / 0 ? void 0 : limit;
									var index = query.index,
										range = query.range;
									var store = trans.objectStore(tableName);
									var source = index.isPrimaryKey ? store : store.index(index.name);
							...
							getMany: function(_a) {
								...
								return new Promise((function(resolve, reject) {
									...
									var successHandler = function(event) {
										...
										if (null != (result[req._pos] = req.result)) ++valueCount;
				...
				function setApiOnPlace(db, objs, tableNames, dbschema) {
					tableNames.forEach((function(tableName) {
						...
						objs.forEach((function(obj) {
							...
							if (!propDesc || "value" in propDesc && void 0 === propDesc.value)
				...
				function deleteRemovedTables(newSchema, idbtrans) {
					for (var i = 0; i < idbtrans.db.objectStoreNames.length; ++i) {
						...
						if (null == newSchema[storeName]) idbtrans.db.deleteObjectStore(storeName);
					}
				}
				...
				function buildGlobalSchema(db, idbdb, tmpTrans) {
					...
					dbStoreNames.forEach((function(storeName) {
						...
						var keyPath = store.keyPath;
						var primKey = createIndexSpec(nameFromKeyPath(keyPath), keyPath || "", false, false, !!store.autoIncrement, keyPath && "string" !== typeof keyPath, true);
						var indexes = [];
						for (var j = 0; j < store.indexNames.length; ++j) {
							var idbindex = store.index(store.indexNames[j]);
							keyPath = idbindex.keyPath;
							var index = createIndexSpec(idbindex.name, keyPath, !!idbindex.unique, !!idbindex.multiEntry, false, keyPath && "string" !== typeof keyPath, false);
							indexes.push(index);
						}
				...
				function adjustToExistingIndexNames(db, schema, idbtrans) {
					...
					for (var i = 0; i < storeNames.length; ++i) {
						...
						for (var j = 0; j < store.indexNames.length; ++j) {
							...
							var dexieName = "string" === typeof keyPath ? keyPath : "[" + slice(keyPath).join("+") + "]";
							if (schema[storeName]) {
					...
					if ("undefined" !== typeof navigator && /Safari/.test(navigator.userAgent) && !/(Chrome\/|Edge\/)/.test(navigator.userAgent) && _global.WorkerGlobalScope && _global instanceof _global.WorkerGlobalScope && [].concat(navigator.userAgent.match(/Safari\/(\d*)/))[1] < 604) db._hasGetAll = false;
				}
				...
				function DatabaseEnumerator(indexedDB) {
					var hasDatabasesNative = indexedDB && "function" === typeof indexedDB.databases;
					var dbNamesTable;
				...
				function dexieOpen(db) {
					...
					return DexiePromise.race([state.openCanceller, new DexiePromise((function(resolve, reject) {
						...
						var dbName = db.name;
						var req = state.autoSchema ? indexedDB.open(dbName) : indexedDB.open(dbName, Math.round(10 * db.verno));
					...
					function step(getNext) {
						return function(val) {
							var next = getNext(val),
								value = next.value;
							return next.done ? value : !value || "function" !== typeof value.then ? isArray(value) ? Promise.all(value).then(onSuccess, onError) : onSuccess(value) : value.then(onSuccess, onError);
						};
					}
					...
					return DexiePromise.resolve().then((function() {
						...
						var promiseFollowed = DexiePromise.follow((function() {
							...
							if (returnValue)
								...
								} else if ("function" === typeof returnValue.next && "function" === typeof returnValue.throw) returnValue = awaitIterator(returnValue);
						}), zoneProps);
					...
					return __assign(__assign({}, down), {
						table: function(tableName) {
							...
							function addVirtualIndexes(keyPath, keyTail, lowLevelIndex) {
								...
								var keyLength = null == keyPath ? 0 : "string" === typeof keyPath ? 1 : keyPath.length;
								...
								if (keyLength > 1) {
									var virtualKeyPath = 2 === keyLength ? keyPath[0] : keyPath.slice(0, keyLength - 1);
								...
								openCursor: function(req) {
									...
									function createVirtualCursor(cursor) {
										function _continue(key) {
											null != key ? cursor.continue(pad(key, req.reverse ? down.MAX_KEY : down.MIN_KEY, keyTail)) : req.unique ? cursor.continue(pad(cursor.key, req.reverse ? down.MIN_KEY : down.MAX_KEY, keyTail)) : cursor.continue();
										}
								...
								var tableMiddleware = __assign(__assign({}, downTable), {
									mutate: function(req) {
										...
										function addPutOrDelete(req) {
											...
											req = "add" === req.type || "put" === req.type ? __assign(__assign({}, req), {
												keys: keys$1,
												wantResults: true
											}) : __assign({}, req);
											if ("delete" !== req.type) req.values = __spreadArrays(req.values);
											...
											return getExistingValues(downTable, req, keys$1).then((function(existingValues) {
												var contexts = keys$1.map((function(key, i) {
													...
													if ("delete" === req.type) deleting.fire.call(ctx, key, existingValue, dxTrans);
													else if ("add" === req.type || void 0 === existingValue) {
														var generatedPrimaryKey = creating.fire.call(ctx, key, req.values[i], dxTrans);
														if (null == key && null != generatedPrimaryKey) {
												...
												return downTable.mutate(req).then((function(_a) {
													...
													for (var i = 0; i < keys$1.length; ++i) {
														...
														var ctx = contexts[i];
														if (null == primKey) ctx.onerror && ctx.onerror(failures[i]);
														else ctx.onsuccess && ctx.onsuccess("put" === req.type && existingValues[i] ? req.values[i] : primKey);
													}
													return {
														failures,
														results,
				...
				var Dexie = function() {
					...
					Dexie.prototype.version = function(versionNumber) {
						...
						versionNumber = Math.round(10 * versionNumber) / 10;
					...
					Dexie.prototype._transaction = function(mode, tables, scopeFunc) {
						...
						if (!parentTransaction || parentTransaction.db !== this || -1 !== mode.indexOf("!")) parentTransaction = null;
						var onlyIfCompatible = -1 !== mode.indexOf("?");
						...
						try {
							...
							if ("r" == mode || mode === READONLY) idbMode = READONLY;
							else if ("rw" == mode || mode == READWRITE) idbMode = READWRITE;
				...
				props(Dexie$1, __assign(__assign({}, fullNameExceptions), {
					...
					async: function(generatorFn) {
						return function() {
							try {
								...
								if (!rv || "function" !== typeof rv.then) return DexiePromise.resolve(rv);
								return rv;
							} catch (e) {
								return rejection(e);
							}
					...
					waitFor: function(promiseOrFunction, optionalTimeout) {
						var promise = DexiePromise.resolve("function" === typeof promiseOrFunction ? Dexie$1.ignoreTransaction(promiseOrFunction) : promiseOrFunction).timeout(optionalTimeout || 6e4);
				...
				db.open();
				const users = [],
					customPronouns = Settings.get("customPronouns");
				for (const userId in customPronouns) users.push({
					id: userId,
				...
				class PronounsDB {
					static bulkLoad(userIds) {
						return new Promise((resolve => {
							...
							external_PluginApi_namespaceObject.Logger.debug(`Performing fetch for [${idsMap.join(", ")}]`);
							external_require_https_default().get(Endpoints.LOOKUP_BULK.format({
								userIds: idsMap.join(",")
							}), {
								headers: {
									"User-Agent": "BetterDiscord PronounDB Plugin v" + package_namespaceObject.um.i8
								}
							}, (res => {
								let chunks = [];
								...
								res.on("end", (() => {
									try {
										...
										if (!allUsers || 0 === Object.keys(allUsers).length) return;
										let users = userIds.map((e => ({
											id: e,
											expiresAt: Date.now() + 30 * 6e4,
											pronouns: _nullishCoalesce(allUsers[e], (() => null))
										})));
										db.pronouns.bulkPut(users);
										for (const {
												id
											...
											external_PluginApi_namespaceObject.Logger.info("Updating for", id);
											dispatcher.update.fire(id);
										}
										resolve(users);
									} catch (error) {
										external_PluginApi_namespaceObject.Logger.error("Fetch error: ", error);
									}
								}));
							}));
						}));
					...
					static async getPronouns(userId, request = true) {
						...
						if (!user && request) return this.fetchPronouns(userId);
						if (!user) return null;
						if (Date.now() >= user.expiresAt && 0 !== user.expiresAt) return this.fetchPronouns(userId);
					...
					static async setPronouns(userId, pronouns) {
						...
						Settings.set("customPronouns", Object.assign({}, Settings.get("customPronouns"), {
							[userId]: pronouns
						}));
					}
					...
					static connect(Component) {
						return props => {
							const [data, setData] = (0, external_BdApi_React_.useState)(null);
							(0, external_BdApi_React_.useEffect)((() => {
								const handler = userId => {
									if (userId !== props.userId) return;
									this.getPronouns(userId).then((data => {
										setData(data);
									})).catch(external_PluginApi_namespaceObject.Logger.error);
								};
				...
				var pronouns = __webpack_require__(793);
				var pronouns_React = __webpack_require__(832);
				const components_pronouns = Settings.connectStore(PronounsDB.connect((function Pronoun({
					data,
					render,
					type
				}) {
				...
				var style = __webpack_require__(577);
				...
				const createUpdateWrapper_createUpdateWrapper = (Component, valueProp = "value", changeProp = "onChange", valueIndex = 0) => props => {
				...
				function src_nullishCoalesce(lhs, rhsFn) {
					if (null != lhs) return lhs;
					else return rhsFn();
				}
				...
				class PronounDB extends(external_BasePlugin_default()) {
					constructor(...args) {
						super(...args);
						PronounDB.prototype.__init.call(this);
						PronounDB.prototype.__init2.call(this);
					}
					__init() {
						this.promises = {
							cancelled: false,
					...
					async patchMessageTimestamp() {
						const OriginalMessageTimestamp = external_PluginApi_namespaceObject.WebpackModules.getModule((m => _optionalChain([m, "optionalAccess", _2 => _2.default, "optionalAccess", _3 => _3.toString, "call", _4 => _4(), "access", _5 => _5.indexOf, "call", _6 => _6("showTimestampOnHover")]) > -1));
						this.patches.push(external_PluginApi_namespaceObject.Patcher.after(OriginalMessageTimestamp, "default", ((_, [{
							message: {
								author: user
							}
						}], ret) => {
						...
						const Modules = external_PluginApi_namespaceObject.WebpackModules.findAll((m => ~["ChannelMessage", "InboxMessage"].indexOf(_optionalChain([m, "optionalAccess", _7 => _7.type, "optionalAccess", _8 => _8.displayName]))));
						for (const Module of Modules) {
							const unpatch = external_PluginApi_namespaceObject.Patcher.after(Module, "type", ((_, __, ret) => {
								const tree = external_PluginApi_namespaceObject.Utilities.findInReactTree(ret, (m => _optionalChain([m, "optionalAccess", _9 => _9.childrenHeader])));
								if (!tree) return;
					...
					async patchUserPopout() {
						...
						this.patches.push(external_PluginApi_namespaceObject.Patcher.after(UserPopoutBody, "default", ((_, [{
							...
							if (!Array.isArray(_optionalChain([res, "optionalAccess", _10 => _10.props, "optionalAccess", _11 => _11.children])) || res.props.children.some((s => _optionalChain([s, "optionalAccess", _12 => _12.type]) === components_pronouns))) return;
					...
					async patchUserContextMenus() {
						const Menus = external_PluginApi_namespaceObject.WebpackModules.findAll((m => _optionalChain([m, "access", _13 => _13.default, "optionalAccess", _14 => _14.displayName, "optionalAccess", _15 => _15.search, "call", _16 => _16(/user.*contextmenu/i)]) > -1));
						const SelectOptions = Object.entries(Pronouns).reduce(((items, [key, value]) => {
							items.push({
								label: src_nullishCoalesce(value, (() => key)),
								value: key
							});
							return items;
						}), []);
