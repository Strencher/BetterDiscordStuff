...
function buildPlugin([BasePlugin, PluginApi]) {
	...
	(() => {
		...
		(() => {
							...
							res.on("end", (() => {
								try {
									...
									for (const {
										...
										external_PluginApi_namespaceObject.Logger.debug("Updating for", id);
			...
			var pronouns_React = __webpack_require__(113);
				...
				async patchMessageTimestamp() {
					...
					function PatchedMessageHeader({
						...
						try {
							...
							if (Array.isArray(children)) children.push(external_BdApi_React_default().createElement(components_pronouns, {
								userId: user.id,
