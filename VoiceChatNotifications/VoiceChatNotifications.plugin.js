...
const config = {
	...
	"changelog": [{
		...
		"items": [
			...
			"Button being offcenter has been fixed.",
			"Accidental `console.log` was removed. :eyes:"
		]
	}],
	"build": {
		"zlibrary": true,
...
function buildPlugin([BasePlugin, PluginApi]) {
	...
	(() => {
		...
		(() => {
			...
			const VoiceChatNotifications_package = {
				info: {
					name: "VoiceChatNotifications",
					version: "1.0.1",
					description: "Shows you certain events from voicechats in a logs panel or as desktop notification.",
				...
				changelog: [{
					type: "improved",
					title: "Improvements",
					items: ["Some strings have been changed.", "Button being offcenter has been fixed.", "Accidental `console.log` was removed. :eyes:"]
				}],
				build: {
					zlibrary: true,
					copy: true,
			...
			class Settings {
				static saveState() {
					external_PluginApi_namespaceObject.PluginUtilities.saveSettings(VoiceChatNotifications_package.info.name, this.settings);
				}
			}
			_defineProperty(Settings, "settings", external_PluginApi_namespaceObject.PluginUtilities.loadSettings(VoiceChatNotifications_package.info.name, {}));
