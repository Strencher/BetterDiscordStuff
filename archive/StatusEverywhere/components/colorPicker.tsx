...
export function ComplexColorPicker({ value, defaultValue, onChange, colors = defaultColors }) {
...
export function TinyColorPicker({ value, onChange, className, defaultValue }: { className?: string, onChange: (color: string) => void, value: string, defaultValue: string }) {
    const [color, setColor] = useState(resolveColor(value));
    const handleChange = useCallback(({ target: { value } }) => {
        setColor(value);
        onChange(resolveColor(value));
    ...
    return (
        <div className={joinClassNames(styles.tiny, className)} onContextMenu={() => handleChange({ target: { value: resolveColor(defaultValue) } })}>
            <input type="color" className={styles.input} onChange={handleChange} value={resolveColor(color)} style={{backgroundColor: resolveColor(color)}} />
            <Dropper />
        </div>
    );
};